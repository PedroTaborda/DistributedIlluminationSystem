...
CommandParser parser((Command[]){
    {'a', "<int>", "sets the anti-windup state", intArgCom([](int val) {
         if (val < 0 || val > 1) ERR controller.setOccupancy(val);
         ACK
     }),
     NULL},
    {'b', "<int>", "sets the feedback state", intArgCom([](int val) {
         if (val < 0 || val > 1) ERR controller.setFeedback(val);
         ACK
     }),
     NULL},
    {'B', "", "get last minute buffer", NULL,
     (Command[]){{'l', "", "measured lumminance", notImplemented, NULL},
                 {'d', "", "duty cycle", notImplemented, NULL},
                 {'\0', "", "", NULL, NULL}}},
    {'C', "", "calibration utilities", NULL,
     (Command[]){
         {'a', "", "calibrate gamma and tau, save these parameters to EEPROM and activate them",
          noArgCom(calibrateAutoCommand), NULL},
         {'c', "<bool: gamma> <bool: tau>", "calibrates gamma and/or tau and saves the calibrated parameters",
          calibrateCommand, NULL},
         {'s', "<bool: gamma> <bool: tau>",
          "saves gamma and/or tau from calibrated parameters as final (writes to EEPROM)", saveCalibrationCommand,
          NULL},
         {'p', "", "print latest calibrated parameters and active parameters", noArgCom(printCalibratedCommand), NULL},
         {'\0', "", "", NULL, NULL}}},
    {'d', "<float>", "sets the duty cycle", floatArgCom([](float duty) {
         if (duty < 0 || duty > 1) ERR controller.turnControllerOff();
         set_u(duty);
         ACK
     }),
     NULL},
    {'g', "", "get command", NULL,
     (Command[]){
         ...
         {'t', "", "time since restart", printfCom("t %d %lu", myID, to_ms_since_boot(get_absolute_time()) / 1000),
          NULL},
         ...
         {'x', "", "external luminance",
          printfCom("x %d %f", myID, controller.getSample().L - gain * controller.getSample().u), NULL},
         {'\0', "", "", NULL, NULL}}},
    {'h', "", "help", noArgCom(help), NULL},
    {'k', "<int>", "callibrator gain <id> to <id>",
     intArgCom([](int id) { printI2C("k %d %f", myID, calibrator.getGainId(id)) }), NULL},
    {'o', "<int>", "sets the occupancy state", intArgCom([](int val) {
         if (val < 0 || val > 1) ERR controller.setOccupancy(val);
         ACK
     }),
     NULL},
    ...
    {'r', "<float>", "sets the reference", floatArgCom([](float reference) {
         if (reference < 0) ERR controller.setReference(reference);
         ACK
     }),
     NULL},
    {'R', "", "system reset", noArgCom([]() {
         __NVIC_SystemReset();
         ACK
     }),
     NULL},
    {'s', "", "stream variable", NULL,
     (Command[]){{'l', "", "measured lumminance", notImplemented, NULL},
                 {'d', "", "duty cycle", notImplemented, NULL},
                 {'\0', "", "", NULL, NULL}}},
    {'t', "", "time", printfCom("t %d %lu", myID, millis()), NULL},
    {'U', "<float>", "sets the unoccupied lower bound lumminance", notImplemented, NULL},
    {'w', "<int>", "sets the feedforward state", intArgCom([](int val) {
         if (val < 0 || val > 1) ERR controller.setFeedforward(val);
         ACK
     }),
     NULL},
    {'\0', "", "", NULL, NULL}});
...
void setup() {
    ...
    if (comms.my_id == 0) comms.calibrateNetwork();
}

void loop() {
    if (Serial.available() > 0) parseSerial(comms);
